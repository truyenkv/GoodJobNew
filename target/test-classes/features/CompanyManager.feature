Feature: Company Manage account checks login on first time.
  ...
  Scenario: Admin authorise and add user successfully
    When Join Working Metrics screen via link is "http://goodjobs-dev.dynagility.us/onboarding?email=cd.0030@yopmail.com&token=cGw5OTVocGY4MGI%3D"
    ...
    And Add a new User account with First Name is "truyen", Last Name is "kieu", email is "truyen.linh.5@yopmail.com", role is user.
